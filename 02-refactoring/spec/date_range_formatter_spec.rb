...
RSpec.describe(DateRangeFormatter) do
  ...
  it "formats a date range with the same month name thats in a different year with the whole month names for start and end dates" do
    formatter = DateRangeFormatter.new("2009-11-1", "2010-11-3")
    expect(formatter.to_s).to eq("1st November 2009 - 3rd November 2010")
  end

  it "formats a date range with the same month name thats in a different year with the whole month names for start and end dates including start and end times" do
    formatter = DateRangeFormatter.new("2009-11-1", "2010-11-3", "10:00", "12:00" )
    expect(formatter.to_s).to eq("1st November 2009 at 10:00 - 3rd November 2010 at 12:00")
  end
end
